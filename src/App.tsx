...
const App: React.FC = () => {
  ...
  // ── Game state (uid drives Firestore sync) ───────────────────
  const uid = firebaseUser !== 'loading' && firebaseUser !== null ? firebaseUser.uid : undefined;
  const {
    currentLevelId,
    levelConfig,
    grid,
    ...
    handleTileSwap,
    resetProgress
  } = useGameState(uid) as any; // Cast for simpler swap access
  ...
  if (firebaseUser === 'loading') {
    return (
      <div className="app-viewport" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#030712' }}>
    ...
    <div className="app-viewport">
      ...
      <header className="app-header">
        ...
        <div className="header-right-panel">
          ...
          <button
            className="sound-toggle-btn signout-btn"
            onClick={() => logoutUser()}
